...
public class AccountCreat : MonoBehaviour
{
    void Start()
    {
        SaverDatas.p_Data.c_UserDatas = SaverDatas.c_Datas;
        for(var i = 0; i < 6; i++)
        {
            SaverDatas.p_Data.team[i] = SaverDatas.p_Data.c_UserDatas[i];
        }

        SaverDatas.p_Data.userRank = 1;
        SaverDatas.p_Data.userTicket = 0;
        SaverDatas.p_Data.userExp = 0;
        SaverDatas.p_Data.userMoney = 0;
    }

    [SerializeField] private InputField nameSpace;
    [SerializeField] private GameObject text_error;

    private bool isSet = false;
    public void Click()
    {
        string name = nameSpace.text;
        bool isMatch = false;
        ...
        if (isMatch)
        {
            text_error.GetComponent<Text>().text = "この名前は使用することが出来ません";
        ...
        else
        {
            ...
            {
                text_error.GetComponent<Text>().text = "文字数が足りません";
                StartCoroutine(falseMessage());
            }
        }
    }

    private void Decision_Name(string name)
    {
        SaverDatas.p_Data.userName = name;
    ...
    private void UpdataUserData()
    {
        PlayFabClientAPI.UpdateUserData(
            new UpdateUserDataRequest
            {
                ...
                {
                    {"PlayerData", PlayFabSimpleJson.SerializeObject(SaverDatas.p_Data) },
                }
            },
            result =>
            {
    ...
    IEnumerator falseMessage()
    {
        yield return new WaitForSeconds(1.5f);
        text_error.GetComponent<Text>().text = "";
    }
